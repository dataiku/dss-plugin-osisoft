...
app.controller('AfExplorerFormCtrl', [
  ...
  function ($scope, $stateParams, $q, CodeMirrorSettingService, TreeDataService) {
    ...
    $scope.hasPreset = function () {
      return $scope.config.credentials?.mode !== 'NONE' && $scope.config.credentials.name
    }
    ...
    $scope.$watchGroup(
      [
        function () {
          return $scope.config?.credentials ? $scope.config.credentials.mode : null;
        },
        function () {
          return $scope.config?.credentials ? $scope.config.credentials.name : null;
        }
      ],
    ...
    $scope.onSearchInputKeydown = function ($event) {
      if ($event && ($event.key === "Enter" || $event.keyCode === 13)) {
        ...
        if ($event.target?.id === "ReturnsName") {
    ...
    function getAllTemplatedAttributes() {
      ...
      templateGroups.forEach(group => {
        ...
        templatedAttributes.push(group.attributes);
      });
...
app.component('treeNode', {
  ...
  controller: function () {
    ...
    ctrl.onNodeClick = function (node) {
      ...
      const hasActiveAttributeSearch = !!ctrl?.config?.attribute_name.trim();
